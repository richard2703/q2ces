@extends('layouts.main', ['activePage' => 'equipos', 'titlePage' => __('Lista de Unidades del SAT')])
@section('content')
    <div class="content">
        @if ($errors->any())
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header bacTituloPrincipal">
                            <h4 class="card-title">Unidades SAT</h4>
                        </div>
                        <div class="card-body">
                            @if (session('success'))
                                    ...
                                    <div class="col-12 text-right">
                                        ...
                                        @can('catalogos_create')
                                            <button class="btn botonGral float-end" data-bs-toggle="modal"
                                                ...
                                                Añadir una Unidad SAT
                                            </button>
                                        @endcan
                                    </div>
                                </div>
                                <div class="divBorder">
                                    <p>Catálogo General de Unidades del SAT, Es Utilizado para registrar las claves del SAT con la finalidad de relacionarlo a la seccion de conceptos.</p>
                                </div>
                            </div>
                                                ...
                                                <td class="td-actions text-center">
                                                    @can('catalogos_show')
                                                        <a href="#" data-bs-toggle="modal" data-bs-target="#editarItem"
                                                            onclick="cargaItem('{{ $item->id }}','{{ $item->nombre }}','{{ $item->codigo }}','{{ $item->comentario }}','{{ true }}')">
                                                    ...
                                                    @can('catalogos_edit')
                                                        <a href="#" class="" data-bs-toggle="modal"
                                                            ...
                                                            onclick="cargaItem('{{ $item->id }}','{{ $item->nombre }}','{{ $item->codigo }}','{{ $item->comentario }}','{{ false }}')">
                                                            <svg xmlns="http://www.w3.org/2000/svg " width="28"
                                                                height="28" fill="currentColor"
                                                    ...
                                                    @can('catalogos_destroy')
                                                        <form action="{{ route('unidadesSat.destroy', $item->id) }}"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bacTituloPrincipal">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">&nbsp Añadir Unidad SAT</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="row d-flex" action="{{ route('unidadesSat.store') }}" method="post">
                        @csrf
    ...
    <div class="modal fade" id="editarItem" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bacTituloPrincipal">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">&nbsp <span id="tituloModal">Editar</span> Unidad del SAT</label></h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="row d-flex" action="{{ route('unidadesSat.update') }}" method="post">
                        @csrf
                        @method('post')
                        ...
                        <div class="col-12 col-sm-6 mb-3">
                            <label class="labelTitulo">Código: <span>*</span></label></br>
                            <input type="text" class="inputCaja" id="controlCodigo" name="codigo" value="">
                        </div>
    ...
    <script>
        function cargaItem(id, nombre, codigo, comentario, modalTipo) {
