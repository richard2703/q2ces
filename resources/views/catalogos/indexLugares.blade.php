...
@section('content')
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="divBorder">
                                    <p>Catálogo General de Lugares, Es Utilizado para Asignar un Lugar (Ej. Taller) y en
                                        Donde se Podrán Asignar las Ubicaciones (Ej. Oficinas) de los Extintores.</p>
                                </div>
                            </div>
                                        ...
                                        @forelse ($lugares as $item)
                                            <tr>
                                                ...
                                                <td class="td-actions text-center">
                                                    ...
                                                    @endcan
                                                    @can('lugares_destroy')
                                                        <form action="{{ route('lugares.destroy', $item->id) }}" method="POST"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bacTituloPrincipal">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">&nbsp Añadir Lugar</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="row d-flex" action="{{ route('lugares.store') }}" method="post">
                        @csrf
                        ...
                        <div class=" col-3 col-sm-4 col-lg-2 mb-3 ">
                            ...
                            <input class="form-check-input is-valid align-self-end mb-2" type="checkbox" name="check"
                                id="controlCheckNew" checked style="font-size: 20px;">
                        </div>

                        <div class=" col-12 mb-3 ">
                            <label class="labelTitulo">Lugares:
                                <span>*</span></label></br>
                            <select id="ubicacionId" name="ubicacionId" class="form-select" required
                        ...
                        <div class=" col-12  mb-3 ">
                            ...
                            <textarea class="form-control border-green" placeholder="Escribe tu comentario aquí" id="floatingTextarea"
                ...
                <div class="modal-body">
                    <form class="row d-flex" action="{{ route('lugares.update', 0) }}" method="post">
                        ...
                        <div class=" col-12 mb-3 ">
                            <label class="labelTitulo">Lugares:
                                <span>*</span></label></br>
                            <select id="controlUbicacion" name="ubicacionId" class="form-select" required
                                aria-label="Default select example">
                                <option value="">Seleccione</option>
                                @foreach ($ubicacion as $item)
                                    <option value="{{ $item->id }}">
    ...
    <script>
        function cargaItem(id, nombre, ubicacion, comentario, activo, modalTipo) {
            ...
            const txtUbicacion = document.getElementById('controlUbicacion');
            txtUbicacion.disabled = modalTipo;
            // txtUbicacion.options.forEach(element => {
            //     console.log('element',element);
            // });
            for (let i = 0; i < txtUbicacion.options.length; i++) {
                if (txtUbicacion.options[i].value == ubicacion) {
                    // txtUbicacion.options[i].selected = true;
                    txtUbicacion.selectedIndex = i;
                    console.log('element', txtUbicacion.options[i].value);
                }
            }
            txtUbicacion.disabled = modalTipo;
